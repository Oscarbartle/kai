...
pub struct ShoppingListLine {
    ...
    /// unambiguous — see `add_item`'s merge rule. Lets the UI re-group a
    /// recipe's lines under one card after reopening the list, instead
    /// of only within the session that added them.
